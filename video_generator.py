...
def download_font():
    ...
    try:
        ...
        log(f"WARNING: Could not download Montserrat font ({e}). Falling back to system default.")
        return "Arial"

def generate_voiceover(text, output_file="voiceover.mp3"):
    log("Synthesizing neural voiceover via edge-tts...")
    # Clean text to prevent odd pronunciations of quotes or breaks
    clean_text = text.replace("\n", " ").replace("\"", "").replace("—", "by")
    
    # 100% Free, realistic studio-grade voice wrapper
    voice = "en-US-AndrewNeural"
    
    try:
        subprocess.run(
            ["edge-tts", "--text", clean_text, "--write-media", output_file, "--voice", voice],
            check=True
        )
        log("Voiceover generated successfully.")
    except Exception as e:
        log(f"ERROR generating voiceover: {e}")
        sys.exit(1)
...
def download_background_video(output_file="background.mp4"):
    ...
    try:
        ...
        log(f"ERROR downloading from Mixkit: {e}")
...
def render_final_video(quote_text, font_path, voice_duration, video_file="background.mp4", audio_file="voiceover.mp3", output_file="final_post.mp4"):
    ...
    # Secure escape character sequences for the FFmpeg filter
    escaped_text = (
        wrapped_quote
        .replace("\\", "\\\\")
        .replace(":", "\\:")
    ...
    style_filter = "fontsize=46:fontcolor=white:borderw=5:bordercolor=black:x=(w-text_w)/2:y=(h-text_h)/2:line_spacing=12"
    ...
    try:
        ...
        try:
            ...
            fallback_cmd = [
                ...
                "-i", audio_file,
                "-t", f"{voice_duration + 1.0:.2f}",
                "-vf", f"drawtext=text='{escaped_text}':fontsize=40:fontcolor=white:borderw=4:bordercolor=black:x=(w-text_w)/2:y=(h-text_h)/2,scale=1080:1920",
            ...
            log(f"CRITICAL ERROR: Render pipeline failed: {ex}")
            sys.exit(1)
...
def post_to_tiktok_via_buffer(video_url, quote_text):
    ...
    try:
        log("Retrieving connected social profiles...")
        profiles_res = requests.get("https://api.bufferapp.com/1/profiles.json", headers=headers, timeout=20)
        ...
        log("Sending payload to Buffer...")
        post_res = requests.post("https://api.bufferapp.com/1/updates/create.json", headers=headers, data=post_payload, timeout=30)
        post_res.raise_for_status()
        
        log("SUCCESS! Video added to Buffer queue and pushed to TikTok!")
        return True
    except Exception as e:
...
def clean_temp_files():
    ...
    temp_files = ["voiceover.mp3", "background.mp4"]
    for f in temp_files:
        if os.path.exists(f):
            os.remove(f)
            log(f"Deleted: {f}")
...
if __name__ == "__main__":
    ...
    if direct_link:
        ...
        log("Skipping automated post because direct cloud link was not generated. Output video saved locally as 'final_post.mp4'.")
