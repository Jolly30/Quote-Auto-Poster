...
def render_final_video(quote_text, font_path, voice_duration, video_file="background.mp4", audio_file="voiceover.mp3", output_file="final_post.mp4"):
    ...
    if has_music:
        ...
        # Voiceover volume=1.0, music volume=0.12 (soft background)
        # amix inputs=2:duration=first makes it end when the voiceover ends
        cmd.extend([
            "-filter_complex", "[1:a]volume=1.0[v_audio];[2:a]volume=0.12[bg_audio];[v_audio][bg_audio]amix=inputs=2:duration=first:dropout_transition=2[a]",
            "-map", "0:v",
            "-map", "[a]"
        ])
