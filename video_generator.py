...
# ==========================================
# Font size for centered motivational captions (e.g. 72 for bold, readable text on phones)
FONT_SIZE = int(os.environ.get("FONT_SIZE", "72"))

# Max character width per line before wrapping (narrow wrap of 20 allows large text sizes)
TEXT_WRAP_WIDTH = int(os.environ.get("TEXT_WRAP_WIDTH", "20"))
...
def render_final_video(quote_text, font_path, voice_duration, video_file="background.mp4", audio_file="voiceover.mp3", output_file="final_post.mp4"):
    ...
    style_filter = f"fontsize={FONT_SIZE}:fontcolor=white:borderw=6:bordercolor=black:x=(w-text_w)/2:y=(h-text_h)/2:line_spacing=16:expansion=none"
    ...
    try:
        ...
        try:
            ...
            fallback_cmd.extend([
                "-t", f"{voice_duration + 1.0:.2f}",
                "-vf", f"drawtext={fallback_text_filter}:fontsize={FONT_SIZE - 6}:fontcolor=white:borderw=5:bordercolor=black:x=(w-text_w)/2:y=(h-text_h)/2:expansion=none,scale=1080:1920"
            ])
            if has_music:
                fallback_cmd.extend([
                    "-filter_complex", f"[1:a]volume={VOICE_VOLUME}[v_audio];[2:a]volume={MUSIC_VOLUME}[bg_audio];[v_audio][bg_audio]amix=inputs=2:duration=first:dropout_transition=2[a]",
