...
def upload_to_catbox(file_path):
    log("Uploading compiled video to Catbox.moe (100% Free Public CDN)...")
    ...
    try:
        ...
        log(f"WARNING: Catbox.moe upload failed ({e}). Trying Litterbox fallback...")
        # Try Litterbox as a temporary fallback (lasts 24h)
        ...
        try:
            ...
            log(f"ERROR: Litterbox fallback also failed ({ex}).")
            return None
...
if __name__ == "__main__":
    ...
    # 6. Upload compiled video to free cloud hosting (GitHub Releases for public repos, Catbox for private/local)
    ...
    if visibility == "public":
        ...
        if not direct_link:
            log("GitHub Release upload failed. Trying Catbox fallback...")
            direct_link = upload_to_catbox(final_video)
    elif visibility == "private":
        log("GitHub Repository is PRIVATE. Release assets will NOT be accessible to Buffer.")
        log("Uploading video to Catbox.moe public CDN instead...")
        direct_link = upload_to_catbox(final_video)
    else:
        # Local run
